...
def write_coefficients_to_file(model, feature_names, output_filename):
    ...
    with open(output_filename, 'w') as csvfile:
        csvfile.write('product_id\tproduct_name\tCTO\tIT Director\tVP of IT\tCIO\n')
        for i in range(len(feature_names)):
            csvfile.write('{}\t{}\t{}\t{}\t{}\n'.format(feature_names[i], feature_dict[feature_names[i]][0], feature_dict[feature_names[i]][1], feature_dict[feature_names[i]][2], feature_dict[feature_names[i]][3]))




def main():
    results_filename = 'inputs/within_persona_classifier/it_exec/results.json';
    title_filenames = [
    ...
    # write_coefficients_to_file(model, feature_names, 'outputs/within_persona_classifier/it_exec/it_exec_multi_LG_coef.txt')
