...
def write_coefficients_to_file(model, feature_names, output_filename):
    ...
    with open(output_filename, 'w') as csvfile:
        csvfile.write('product_id\tproduct_name\tCFO\tChief Procurement Officer\tDirector of Finance\tDirector of Supply Management\tFinance Manager\tFinancial Advisor\tProcurement Manager\tPurchasing Director\tPurchasing Manager\tVP of Accounting\tVP of Finance\n')
        for i in range(len(feature_names)):
            csvfile.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(feature_names[i], feature_dict[feature_names[i]][0], feature_dict[feature_names[i]][1], feature_dict[feature_names[i]][2], feature_dict[feature_names[i]][3], feature_dict[feature_names[i]][4], feature_dict[feature_names[i]][5], feature_dict[feature_names[i]][6], feature_dict[feature_names[i]][7], feature_dict[feature_names[i]][8], feature_dict[feature_names[i]][9], feature_dict[feature_names[i]][10]))




def main():
    results_filename = 'inputs/within_persona_classifier/finance/results.json';
    title_filenames = [
    ...
    # write_coefficients_to_file(model, feature_names, 'outputs/within_persona_classifier/finance/finance_multi_LG_coef.txt')
